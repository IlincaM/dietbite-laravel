...
class WP_Import extends WP_Importer {
	...
	var $gt3_menu_item_orphans = array();
...
<?php
	...
	function process_menu_item( $item ) {
		...
		$gt3_menu_id = term_exists( $menu_slug, 'nav_menu' );
		if ( ! $gt3_menu_id ) {
			printf( __( 'Menu item skipped due to invalid menu slug: %s', 'wordpress-importer' ), esc_html( $menu_slug ) );
			echo '<br />';
			return;
		} else {
			$gt3_menu_id = is_array( $gt3_menu_id ) ? $gt3_menu_id['term_id'] : $gt3_menu_id;
		}
		...
		$id = wp_update_nav_menu_item( $gt3_menu_id, 0, $args );
