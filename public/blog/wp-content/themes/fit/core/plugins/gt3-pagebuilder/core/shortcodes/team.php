...
class ourteam
{
    public function register_shortcode($shortcodeName)
    {
        function shortcode_ourteam($atts, $content = null)
        {
            ...
            $GLOBALS['showOnlyOneTimeJS']['team_icons'] = "
			<script>
				jQuery(document).ready(function($) {
					jQuery('.team_icons_wrapper').each(function(){
						jQuery(this).width(jQuery(this).find('a').size()*40);
						jQuery(this).parent('.team_icons').width(jQuery(this).find('a').size()*40+40);
					});
					jQuery('.icons_toggler').click(function(){
						jQuery(this).parent('.team_icons').toggleClass('hided');
        ...
        <div class="team_slider">
            ...
            while ($wp_query->have_posts()) : $wp_query->the_post();
                ...
                if (has_post_thumbnail()) {
                    $compile .= '<img src="' . aq_resize($featured_image[0], "540", "392", true, true, true) . '" alt="' . get_the_title() . '" />';
                }
                ...
                $compile .= '
					...
					if (is_array($socicons)) {
						...
						foreach ($socicons as $key => $value) {
							...
							$compile .= '<a href="'.$value['link'].'" class="teamlink" title="'.$value['name'].'" style="color:#'.$value['fcolor'].'; background:#'.$value['bcolor'].'"><span><i class="'.$value['data-icon-code'].'"></i></span></a>';
						}
						$compile .= '</div>
						<a href="'.esc_js("javascript:void(0)").'" class="icons_toggler"></a>';
					}
						$compile .= '	</div>
									</div>
