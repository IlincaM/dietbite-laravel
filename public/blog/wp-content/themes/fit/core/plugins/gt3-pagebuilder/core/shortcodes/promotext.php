    ...
    public function register_shortcode($shortcodeName)
    {
        function shortcode_promo_text($atts, $content = null)
        {
			...
			if ($icon_type == "") {
				...
				$button_ico = '<span class="ico_wrapper"><i class="'.$icon_type.'"></i></span>';
				$hasIcon = 'hasIcon';
			}
				...
				<div class="promoblock_wrapper">
					<div class="promo_text_block">
						<div class="promo_text_block_wrapper">
							' . (isset($main_text) ? "<h3 ".(strlen($main_text_color)>0 ? "style='color:#".$main_text_color." !important;'" : "").">" . $main_text . "</h3>" : '') . '
							' . (isset($additional_text) ? "<h5 ".(strlen($additional_text_color)>0 ? "style='color:#".$additional_text_color." !important;'" : "").">" . $additional_text . "</h5>" : '') . '
						</div>
					</div>
						' . ((strlen($button_link) > 0 && strlen($button_text) > 0) ? '<div class="promo_button_block"><a href="'.$button_link.'" target="'. $button_target .'" class="'. $hasIcon .' promo_button shortcode_button '.$button_type.'">'.$button_ico . $button_text.'</a></div>' : '') . '
				</div>
			</div>
            ';
