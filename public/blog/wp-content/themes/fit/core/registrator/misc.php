...
function side_bg_settings_meta_box_cb( $post )
{
    ...
    $available_layouts = array("default" => "Default", "clean" => "Clean", "boxed" => "Pattern or Color", "bgimage" => "Image");
	echo '<div class="simple_portfolio_options">';
		echo '<div class="sidebar_option">Background Settings:<br><select name="pagebuilder[page_settings][page_layout][layout_type]" class="admin_newselect page_layout">';
	...
	if ($post->post_type == 'port'){
		...
		$available_variants = array("default" => "Default", "simple-portfolio-post" => "Simple", "fw-portfolio-post" => "Fullscreen");
		...
		<script>
			jQuery(document).ready(function(){
				pf_default = '".gt3_get_theme_option('default_portfolio_style')."'
				jQuery('.pf_style_select').change(function(){
					if (jQuery(this).val() == 'simple-portfolio-post') {
						jQuery('.simple_portfolio_options').show();
					}
					if (jQuery(this).val() == 'default' && pf_default == 'simple-portfolio-post') jQuery('.simple_portfolio_options').show();
					if (jQuery(this).val() == 'default' && pf_default == 'fw-portfolio-post') jQuery('.simple_portfolio_options').hide();
					if (jQuery(this).val() == 'fw-portfolio-post') jQuery('.simple_portfolio_options').hide();
				});
			});
		</script>";
	}
	...
	if ($post->post_type == 'post'){
		echo '<div class="custom_select_bcarea">
		<span class="htitle">Post Style:</span><select name="pagebuilder[settings][blogpost_style]" class="admin_newselect bp_style_select">';
		$available_variants = array("default" => "Default", "simple-blogpost" => "Simple", "fw-blogpost" => "Fullscreen");
		foreach ($available_variants as $var_id => $var_caption) {
			echo "<option ".((isset($gt3_theme_pagebuilder['settings']['blogpost_style']) && $gt3_theme_pagebuilder['settings']['blogpost_style'] == $var_id) ? 'selected="selected"' : '')." value='$var_id'>$var_caption</option>";
		}
		echo '</select>';
		echo "
		</div>
		<script>
			jQuery(document).ready(function(){
				bp_default = '".gt3_get_theme_option('default_blogpost_style')."'
				jQuery('.bp_style_select').change(function(){
					if (jQuery(this).val() == 'simple-blogpost') {
						jQuery('.simple_portfolio_options').show();
					}
					if (jQuery(this).val() == 'default' && bp_default == 'simple-blogpost') jQuery('.simple_portfolio_options').show();
					if (jQuery(this).val() == 'default' && bp_default == 'fw-blogpost') jQuery('.simple_portfolio_options').hide();
					if (jQuery(this).val() == 'fw-blogpost') jQuery('.simple_portfolio_options').hide();
				});
			});
		</script>";
	}
